...
must_haves:
  truths:
    ...
    - "BluetoothAudioModel 可正確解析原生端返回的 Map 資料（包含 bitrate 和 bluetoothVersion）"
...
注意：
- 此套件需要 Android 專案的額外設定，但權限宣告已在 Plan 01 中處理。
- **專案已包含 device_info_plus 依賴（用於檢查 Android 版本），無需額外新增。**
  </action>
  <verify>
```bash
flutter pub get && flutter pub deps | grep -E "permission_handler|device_info_plus"
```
預期輸出顯示 permission_handler 和 device_info_plus 都已存在
  </verify>
  <done>permission_handler ^11.0.0 已加入 pubspec.yaml 並成功安裝，確認 device_info_plus 已存在於專案依賴中</done>
</task>
...
預期無錯誤
  ...
  <done>bluetooth_audio_model.dart 存在，包含 BluetoothDeviceInfo（含 bluetoothVersion 欄位）、BluetoothCodecInfo（含 bitrate 欄位）、BluetoothAudioInfo 三個類別，每個都有 fromMap factory constructor</done>
</task>
...
預期無錯誤
  ...
  <done>bluetooth_audio_state.dart 和 bluetooth_audio_cubit.dart 存在，Cubit 包含完整的權限檢查、版本檢查（使用專案現有的 device_info_plus）、Platform Channel 呼叫邏輯</done>
</task>
...
- device_info_plus 已確認存在於專案依賴中
- BluetoothAudioModel 可解析原生端返回的 Map（包含 bitrate 和 bluetoothVersion）
