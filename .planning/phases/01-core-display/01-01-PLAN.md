...
must_haves:
  truths:
    ...
    - "Android 端可以取得當前 codec 設定資訊（類型、取樣率、位元深度、聲道模式、位元率）"
...
- 返回包含以下欄位的 Map：
  ...
  - **bitrate: 位元率（使用 getCodecSpecific1() 取得，若無法取得返回 "不支援"）**
...
- **提供 bitrateToString(codecType: Int, codecSpecific1: Long): String 方法**：
  - 對於 LDAC (type=4): 根據 codecSpecific1 值判斷位元率模式
    - 1000: "990 kbps (最高品質)"
    - 660: "660 kbps (標準)"
    - 330: "330 kbps (連接優先)"
    - 0 或 -1: "ABR (自適應)"
  - 對於其他 codec: 返回 "不支援" 或根據已知規格計算

注意事項：
- 使用 try-catch 包裝所有 reflection 呼叫，處理 hidden API 存取失敗
- 處理 A2DP proxy 尚未就緒的情況（儲存 pending result，等 ServiceListener 回調後處理）
- 所有欄位都要做 null 安全處理
- **getCodecSpecific1() 也需要透過 reflection 存取，可能返回 Long 型別**
  </action>
  <verify>
檔案存在且可編譯：
```bash
...
```
  ...
  <done>BluetoothAudioHandler.kt 存在，包含完整的 A2DP proxy 取得邏輯、codec 資訊讀取邏輯（使用 reflection）、常數對照表，以及 bitrate 取得邏輯</done>
</task>
...
- BluetoothAudioHandler 實作完整的 A2DP 資訊讀取邏輯（包含 bitrate）
